...
def register():
    ...
    if reg.validate_on_submit():
        ...
        if User.query.filter_by(email=data['email']).first():
            return render_template('register.html', message="This email already exists", title="Authorization", form=reg)
        if User.query.filter_by(nickname=data['nickname'].lower()).first():
            return render_template('register.html', message="This nickname already exists", title="Authorization", form=reg)
        to_add = User(name=data['name'], nickname=data['nickname'].lower(),
